...
export const translate = async (req, res, next) => {
  try {
    ...
    createHistoryEntry({
      userId: req.user._id, type: "translate",
      inputCode: code, sourceLanguage, targetLanguage, output: result
...
export const analyze = async (req, res, next) => {
  try {
    ...
    createHistoryEntry({
      userId: req.user._id, type: "analyze",
      inputCode: code, sourceLanguage: language, output: result
    }).catch(err => console.error("History save failed:", err));
...
export const optimize = async (req, res, next) => {
  try {
    ...
    createHistoryEntry({
      userId: req.user._id, type: "optimize",
      inputCode: code, sourceLanguage: language, output: result
    }).catch(err => console.error("History save failed:", err));
...
export const explain = async (req, res, next) => {
  try {
    ...
    createHistoryEntry({
      userId: req.user._id, type: "explain",
      inputCode: code, sourceLanguage: language, output: result
    }).catch(err => console.error("History save failed:", err));
